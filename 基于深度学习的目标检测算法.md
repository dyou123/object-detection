...
（很好的参考链接：<https://www.cnblogs.com/skyfsm/p/6806246.html>）
...
在ROI的基础上，又加上了3\*3区域里的9个部分的特征图。每个用来检测对应目标区域，这些特征图叫做**位置敏感得分图（position-sensitive
score map）**。
...
假如有C个类别，我们就有C+1个类，每个类别有3\*3个得分图，

我们就有（C+1）\*3\*3个得分图。每个类别的得分图可以预测该类别的得分，这些得分运用softmax，可计算出每个类别的概率。

![https://images2018.cnblogs.com/blog/1192699/201805/1192699-20180505144433895-646844741.png](media/cf9a9e8da67e6e21613961f305f6704f.png)
...
![avatar](media/6afb24574d27e8f36dc77444b25a53b6.png)

在 Faster R-CNN 中，我们使用卷积核来做 5 个参数的预测：4
个参数对应某个锚点的预测边框，1 个参数对应 objectness 置信度得分。因此 3× 3× D ×
5 卷积核将特征图从 8 × 8 × D 转换为 8 × 8 × 5。

在单次检测器中，卷积核还预测 C
个类别概率以执行分类（每个概率对应一个类别）。因此我们应用一个 3× 3× D × 25
卷积核将特征图从 8 × 8 × D 转换为 8 × 8 × 25（C=20）。
