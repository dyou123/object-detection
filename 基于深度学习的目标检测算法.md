...
4. 使用k-means来选取anchor boxes（在目标检测数据集中对所有目标框进行k-means聚类，发现k=5个anchor box最合适，用kmeans聚类算法来找到合适的anchor box尺寸比例。5种boxes的Avg IOU(61.0)就和Faster R-CNN的9种Avg IOU(60.9)相当。更有代表性）
